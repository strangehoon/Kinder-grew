...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
    ...
    Optional<Attendance> findByStatusAndDate(AttendanceStatus attendanceStatus, LocalDate date);
    ...
    @Query("select a from Attendance a where a.status = :status and month(a.date) = :month and a.child.id = :childId")
    List<Attendance> findByStatusAndChildIdAndMonth(@Param("status") AttendanceStatus status, @Param("childId") Long childId, @Param("month") int month);
